...
//		add_action( 'all_admin_notices', array( $this, 'finish_notice_output_capturing' ), PHP_INT_MAX );
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image'    => esc_html__( 'No preview image defined for this import.', 'maisonco-core' ),
						'dialog_title'             => esc_html__( 'Are you sure?', 'maisonco-core' ),
						'dialog_no'                => esc_html__( 'Cancel', 'maisonco-core' ),
						'dialog_yes'               => esc_html__( 'Yes, import!', 'maisonco-core' ),
						'selected_import_title'    => esc_html__( 'Selected demo import:', 'maisonco-core' ),
						'installing'               => esc_html__( 'Installing...', 'maisonco-core' ),
						'importing'                => esc_html__( 'Importing...', 'maisonco-core' ),
						'successful_import'        => esc_html__( 'Successfully Imported!', 'maisonco-core' ),
						'install_plugin'           => esc_html__( 'Install Plugin', 'maisonco-core' ),
						'installed'                => esc_html__( 'Installed', 'maisonco-core' ),
						'import_failed'            => esc_html__( 'Import Failed', 'maisonco-core' ),
						'import_failed_subtitle'   => esc_html__( 'Whoops, there was a problem importing your content.', 'maisonco-core' ),
						'plugin_install_failed'    => esc_html__( 'Looks like some of the plugins failed to install. Please try again. If this issue persists, please manually install the failing plugins and come back to this step to import the theme demo data.', 'maisonco-core' ),
						'content_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select an XML file for the Content Import.', 'maisonco-core' ),
						'widgets_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select a JSON or WIE file for the Widgets Import.', 'maisonco-core' ),
						'customizer_filetype_warn' => esc_html__( 'Invalid file type detected! Please select a DAT file for the Customizer Import.', 'maisonco-core' ),
						'redux_filetype_warn'      => esc_html__( 'Invalid file type detected! Please select a JSON file for the Redux Import.', 'maisonco-core' ),
					),
				)
			);
	...
	public function upload_manual_import_files_callback() {
		...
		if ( empty( $_FILES ) ) {
			wp_send_json_error( esc_html__( 'Manual import files are missing! Please select the import files and try again.', 'maisonco-core' ) );
		}
		...
		$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'maisonco-core' );
			...
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'maisonco-core' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf( /* translators: %s - the name of the selected import. */
						__( 'The import files for: %s were successfully downloaded!', 'maisonco-core' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'maisonco-core' ) );
			}
		}
	...
	private function final_response() {
		...
		$response['title'] = esc_html__( 'Import Complete!', 'maisonco-core' );
		$response['subtitle'] = '<p>' . esc_html__( 'Congrats, your demo was imported successfully. You can now begin editing your site.', 'maisonco-core' ) . '</p>';
		$response['message'] = '<img class="ocdi-imported-content-imported ocdi-imported-content-imported--success" src="' . esc_url( OCDI_URL . 'assets/images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'maisonco-core' ) . '">';

		if ( ! empty( $this->frontend_error_messages ) ) {
			$response['subtitle'] = '<p>' . esc_html__( 'Your import completed, but some things may not have imported properly.', 'maisonco-core' ) . '</p>';
			$response['subtitle'] .= sprintf(
				wp_kses(
				/* translators: %s - link to the log file. */
					__( '<p><a href="%s" target="_blank">View error log</a> for more information.</p>', 'maisonco-core' ),
					array(
						'p'      => [],
						'a'      => [
							'href'   => [],

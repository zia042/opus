...
class WPCLICommands extends \WP_CLI_Command {
	...
	public function list_predefined() {
		if ( empty( $this->ocdi->import_files ) ) {
			WP_CLI::error( esc_html__( 'There are no predefined demo imports for currently active theme!', 'maisonco-core' ) );
		}

		WP_CLI::success( esc_html__( 'Here are the predefined demo imports:', 'maisonco-core' ) );
	...
	public function import( $args, $assoc_args ) {
		if ( ! $this->any_import_options_set( $assoc_args ) ) {
			WP_CLI::error( esc_html__( 'At least one of the possible options should be set! Check them with --help', 'maisonco-core' ) );
		}
	...
	private function import_predefined( $predefined_index ) {
		if ( ! is_numeric( $predefined_index ) ) {
			WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the OCDI predefined demo import)!', 'maisonco-core' ) );
		}

		$predefined_index = absint( $predefined_index );

		if ( ! array_key_exists( $predefined_index, $this->ocdi->import_files ) ) {
			WP_CLI::warning( esc_html__( 'The supplied predefined index does not exist! Please take a look at the available predefined demo imports:', 'maisonco-core' ) );
		...
		WP_CLI::log( esc_html__( 'Predefined demo import started! All other parameters will be ignored!', 'maisonco-core' ) );
		...
		if ( ! empty( $selected_files['import_file_name'] ) ) { /* translators: %s - the name of the selected demo import. */
			WP_CLI::log( sprintf( esc_html__( 'Selected predefined demo import: %s', 'maisonco-core' ), $selected_files['import_file_name'] ) );
		}

		WP_CLI::log( esc_html__( 'Preparing the demo import files...', 'maisonco-core' ) );

		$import_files =	Helpers::download_import_files( $selected_files );

		if ( empty( $import_files ) ) {
			WP_CLI::error( esc_html__( 'Demo import files could not be retrieved!', 'maisonco-core' ) );
		}

		WP_CLI::log( esc_html__( 'Demo import files retrieved successfully!', 'maisonco-core' ) );

		WP_CLI::log( esc_html__( 'Importing...', 'maisonco-core' ) );
		...
		WP_CLI::log( esc_html__( 'Predefined import finished!', 'maisonco-core' ) );
	}
	...
	private function import_content( $relative_file_path ) {
		...
		if ( ! file_exists( $content_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Content import file provided does not exist! Skipping this import!', 'maisonco-core' ) );
			return false;
		}
		...
		WP_CLI::log( esc_html__( 'Importing content (this might take a while)...', 'maisonco-core' ) );

		Helpers::append_to_file( '', $this->ocdi->log_file_path, esc_html__( 'Importing content' , 'maisonco-core' ) );
		...
		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Content import finished!', 'maisonco-core' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing the content!', 'maisonco-core' ) );
	...
	private function import_widgets( $relative_file_path ) {
		...
		if ( ! file_exists( $widgets_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Widgets import file provided does not exist! Skipping this import!', 'maisonco-core' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing widgets...', 'maisonco-core' ) );

		WidgetImporter::import( $widgets_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Widgets imported successfully!', 'maisonco-core' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing widgets!', 'maisonco-core' ) );
	...
	private function import_customizer( $relative_file_path ) {
		...
		if ( ! file_exists( $customizer_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Customizer import file provided does not exist! Skipping this import!', 'maisonco-core' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing customizer settings...', 'maisonco-core' ) );

		CustomizerImporter::import( $customizer_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Customizer settings imported successfully!', 'maisonco-core' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing customizer settings!', 'maisonco-core' ) );
	...
	private function do_action( $action, $import_files = array(), $all_import_files = array(), $selected_index = null ) {
		if ( false !== Helpers::has_action( $action ) ) { /* translators: %s - the name of the executing action. */
			WP_CLI::log( sprintf( esc_html__( 'Executing action: %s ...', 'maisonco-core' ), $action ) );

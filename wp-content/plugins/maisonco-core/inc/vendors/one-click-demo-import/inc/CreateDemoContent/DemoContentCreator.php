...
class DemoContentCreator {
	...
	public function get_default_content() {
		return array(
			array(
				...
				'name'             => esc_html__( 'About Us', 'maisonco-core' ),
				'description'      => esc_html__( 'Introduce yourself and your business with a clean layout to reassure your leads and customers.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'book-now-page',
				'file'             => OCDI_PATH . 'assets/demo-content/book-now-page.xml',
				'name'             => esc_html__( 'Book Now', 'maisonco-core' ),
				'description'      => esc_html__( 'Expand your reach by accepting appointments online plus detailing your services and staff.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'contact-page',
				'file'             => OCDI_PATH . 'assets/demo-content/contact-page.xml',
				'name'             => esc_html__( 'Contact Us', 'maisonco-core' ),
				'description'      => esc_html__( 'Make it easy to get in touch with you through a completely customizable built-in contact form.', 'maisonco-core' ),
				'required_plugins' => array( 'wpforms-lite' ),
			),
			array(
				'slug'             => 'faq-page',
				'file'             => OCDI_PATH . 'assets/demo-content/faq-page.xml',
				'name'             => esc_html__( 'FAQ', 'maisonco-core' ),
				'description'      => esc_html__( 'Lighten the load on your support team or your inbox by addressing frequently asked questions.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'meet-the-team-page',
				'file'             => OCDI_PATH . 'assets/demo-content/meet-the-team-page.xml',
				'name'             => esc_html__( 'Meet the Team', 'maisonco-core' ),
				'description'      => esc_html__( 'Help potential clients feel more at ease by showing off your hard-working and trustworthy team.', 'maisonco-core' ),
				'required_plugins' => array( 'wpforms-lite' ),
			),
			array(
				'slug'             => 'menu-page',
				'file'             => OCDI_PATH . 'assets/demo-content/menu-page.xml',
				'name'             => esc_html__( 'Menu', 'maisonco-core' ),
				'description'      => esc_html__( 'Display your delicious dishes online to entice website visitors to become restaurant customers.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'portfolio-page',
				'file'             => OCDI_PATH . 'assets/demo-content/portfolio-page.xml',
				'name'             => esc_html__( 'Portfolio', 'maisonco-core' ),
				'description'      => esc_html__( 'Impress leads by visually showcasing your achievements, case studies, and past work.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'services-page',
				'file'             => OCDI_PATH . 'assets/demo-content/services-page.xml',
				'name'             => esc_html__( 'Services', 'maisonco-core' ),
				'description'      => esc_html__( 'Let the world know your services or products\' cost and features in an organized pricing table.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'slug'             => 'shop-page',
				'file'             => OCDI_PATH . 'assets/demo-content/shop-page.xml',
				'name'             => esc_html__( 'Shop', 'maisonco-core' ),
				'description'      => esc_html__( 'Categorize and sell your products online while displaying reviews from happy customers.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
			array(
				'file'             => OCDI_PATH . 'assets/demo-content/testimonials-page.xml',
				'slug'             => 'testimonials-page',
				'name'             => esc_html__( 'Testimonials', 'maisonco-core' ),
				'description'      => esc_html__( 'Tap into the power of social proof by displaying real-life testimonials on your website.', 'maisonco-core' ),
				'required_plugins' => array(),
			),
		);
	}
	...
	public function after_import_wpforms_setup( $slug ) {
		...
		if (
			...
			wp_send_json_error( esc_html__( 'Could not complete the import process for this page. Required WPForms plugin is not activated.', 'maisonco-core' ) );
		}

		if ( ! function_exists( 'wpforms' ) ) {
			wp_send_json_error( esc_html__( 'Could not complete the import process for this page. Required WPForms plugin doesn\'t exist.', 'maisonco-core' ) );
		}

		$form_title = ( $slug === 'meet-the-team-page' ) ? esc_html__( 'Meet the Team Form', 'maisonco-core' ) : esc_html__( 'Contact Form', 'maisonco-core' );
		$form_id = $this->create_wpforms_form( $form_title );

		if ( empty( $form_id ) ) {
			wp_send_json_error( esc_html__( 'Could not complete the import process for this page. Something went wrong while creating a WPForms contact form.', 'maisonco-core' ) );
		}

		$update_page = $this->update_contact_page_form_id( $form_id );

		if ( empty( $update_page ) ) {
			wp_send_json_error( esc_html__( 'Could not complete the import process for this page. Could not update the imported page with correct WPForms form ID.', 'maisonco-core' ) );
		}
	}
	...
	public function import_created_content() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_send_json_error( esc_html__( 'Could not import this page. You don\'t have permission to import content.', 'maisonco-core' ) );
		}

		$slug = ! empty( $_POST['slug'] ) ? sanitize_key( wp_unslash( $_POST['slug'] ) ) : '';

		if ( empty( $slug ) ) {
			wp_send_json_error( esc_html__( 'Could not import this page. Page slug is missing.', 'maisonco-core' ) );
		}
		...
		if ( ! empty( $error ) ) {
			wp_send_json_error(
				sprintf( /* translators: %s - The actual error message. */
					esc_html__( 'An error occured while importing this page: %s', 'maisonco-core' ),
					esc_html( $error )
				)
			);
		}
	...
	private function import_content( $slug ) {
		...
		if ( empty( $import_file ) ) {
			return esc_html__( 'The demo content import file is missing.', 'maisonco-core' );
		}
			...
			array(
				...
				'fields'   => array(
					'0' => array(
						...
						'type'     => 'name',
						'format'   => 'first-last',
						'label'    => esc_html__( 'Name', 'maisonco-core' ),
						'required' => '1',
						'size'     => 'medium',
					),
					'1' => array(
						'id'       => '1',
						'type'     => 'email',
						'label'    => esc_html__( 'Email', 'maisonco-core' ),
						'required' => '1',
						'size'     => 'medium',
					),
					'2' => array(
						'id'          => '2',
						'type'        => 'textarea',
						'label'       => esc_html__( 'Comment or Message', 'maisonco-core' ),
						'description' => '',
						'required'    => '1',
						'size'        => 'medium',
						'placeholder' => '',
				...
				'settings' => array(
					...
					'confirmations'          => array(
						'1' => array(
							'type'           => 'message',
							'message'        => esc_html__( 'Thanks for contacting us! We will be in touch with you shortly.', 'maisonco-core' ),
							'message_scroll' => '1',
						),
					),
					'antispam'               => '1',
					'submit_text'            => esc_html__( 'Submit', 'maisonco-core' ),
					'submit_text_processing' => esc_html__( 'Sending...', 'maisonco-core' ),
				),
			)
		);

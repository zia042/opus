...
class OSF_Field_Agent_Info {
    ...
    private function render_content($userid) {
        ...
        if ($phone) {
            $phone = '<strong>' . __( 'Phone', 'maisonco-core' ) . '</strong>: ' . $phone . '<br>';
        }
        if (!$avatar) {
        ...
        <div class="meta-agent-info">
            ...
            <div class="info">
                ...
                <address>
                    <strong><?php _e( 'Email', 'maisonco-core' ) ?></strong>: <?php echo $user_info->user_email; ?>
                    <br>
                    <?php echo $phone; ?>
                </address>
                <a href="<?php echo esc_url( $link_profile ) ?>"
                   class="button button-primary button-large"><?php _e( 'View Profile', 'maisonco-core' ) ?></a>
                <a href="<?php echo esc_url( $link_properties ) ?>"
                   class="button button-primary button-large"><?php _e( 'View all Properties', 'maisonco-core' ) ?></a>
            </div>
        </div>
        <?php
    }

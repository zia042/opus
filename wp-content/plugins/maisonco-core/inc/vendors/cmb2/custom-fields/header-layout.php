...
class OSF_CMB2_Field_Header_Layout {
    ...
    public function render($field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object) {
        ...
        $option = '<option value="global" selected>'. esc_html__('Global', 'maisonco-core') .'</option>';
        $option .= '<option value="default" selected>'. esc_html__('Default', 'maisonco-core') .'</option>';
        if ($headers){

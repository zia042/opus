...
class OSF_Field_Opal_Upload {
    ...
    public static function upload_user_avatar() {
        ...
        if (!wp_verify_nonce( $nonce, 'allow_uploaded' ) || !$user_id) {
            $ajax_response = array( 'success' => false, 'reason' => __( 'Security check failed!', 'maisonco-core' ) );
    ...
    public static function setup_admin_scripts() {
        ...
        $prop_data = array(
            ...
            'fileTypeTitle' => esc_html__( 'Valid file formats', 'maisonco-core' ),
            'message'       => esc_html__( 'Please enter only digits', 'maisonco-core' ),
        );
    ...
    public static function delete_property_image() {
        ...
        if (!isset( $_POST['property_id'], $_POST['attachment_id'] )) {
            ...
            $ouput->message = __( 'Could not delete this?', 'maisonco-core' );
        ...
        if ($user_id == $author_id) {
            ...
            $ouput->message = __( 'Deleted this done', 'maisonco-core' );
            $ouput->status  = true;
            echo json_encode( $ouput );
            exit;
        }
        ...
        if (!isset( $property_images[$attchment_id] )) {
            $ouput->message = __( 'Could not found this in the collection?', 'maisonco-core' );
            echo json_encode( $ouput );
            exit;
        }
        if (!opalrealestate_is_own_property( $post_id, $user_id )) {
            $ouput->message = __( 'The post is not owned by this user?', 'maisonco-core' );
            echo json_encode( $ouput );
            exit;
        }

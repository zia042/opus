...
abstract class CMB2_REST_Controller extends WP_REST_Controller {
	...
	protected function initiate_rest_read_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_read ) {
			$this->rest_box = new WP_Error( 'cmb2_rest_no_read_error', __( 'This box does not have read permissions.', 'maisonco-core' ), array(
				'status' => 403,
			) );
		}
	}
	...
	protected function initiate_rest_edit_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_edit ) {
			$this->rest_box = new WP_Error( 'cmb2_rest_no_write_error', __( 'This box does not have write permissions.', 'maisonco-core' ), array(
				'status' => 403,
			) );
		}
	}
	...
	protected function initiate_rest_box( $request, $request_type ) {
		...
		if ( ! $this->rest_box ) {
			...
			$this->rest_box = new WP_Error( 'cmb2_rest_box_not_found_error', __( 'No box found by that id. A box needs to be registered with the "show_in_rest" parameter configured.', 'maisonco-core' ), array(
				'status' => 403,
			) );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties'           => array(
				'description' => array(
					'description' => __( 'A human-readable description of the object.', 'maisonco-core' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),
				...
				'name' => array(
					'description' => __( 'The id for the object.', 'maisonco-core' ),
					'type'        => 'integer',
					'context'     => array(
						'view',
					),
				...
				'name' => array(
					'description' => __( 'The title for the object.', 'maisonco-core' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),

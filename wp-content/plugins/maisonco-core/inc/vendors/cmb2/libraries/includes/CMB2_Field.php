...
class CMB2_Field extends CMB2_Base {
	...
	protected function set_field_defaults_group( $args ) {
		$args['options'] = wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'maisonco-core' ),
			'remove_button' => esc_html__( 'Remove Group', 'maisonco-core' ),
		) );
	...
	protected function set_field_defaults_all_or_nothing_types( $args ) {
		...
		$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'maisonco-core' ) : $args['show_option_none'];

		if ( null === $args['show_option_none'] ) {
			$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
			$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'maisonco-core' );
		}
	...
	public function get_cmb() {
		if ( ! $this->cmb_id ) {
			return new WP_Error( 'no_cmb_id', esc_html__( 'Sorry, this field does not have a cmb_id specified.', 'maisonco-core' ) );
		}

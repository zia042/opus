...
class CMB2_hookup extends CMB2_Hookup_Base {
	...
	public function term_hooks() {
		if ( ! function_exists( 'get_term_meta' ) ) {
			wp_die( esc_html__( 'Term Metadata is a WordPress 4.4+ feature. Please upgrade your WordPress install.', 'maisonco-core' ) );
		}

		if ( ! $this->cmb->prop( 'taxonomies' ) ) {
			wp_die( esc_html__( 'Term metaboxes configuration requires a "taxonomies" parameter.', 'maisonco-core' ) );
		}
	...
	public function context_box_title_markup_open( $add_handle = true ) {
		...
		if ( $add_handle ) {
			...
			echo '<button type="button" class="handlediv button-link" aria-expanded="true">';
				echo '<span class="screen-reader-text">' . sprintf( __( 'Toggle panel: %s', 'maisonco-core' ), $title ) . '</span>';

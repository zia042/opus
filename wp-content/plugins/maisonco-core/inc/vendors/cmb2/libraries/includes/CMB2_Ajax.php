...
class CMB2_Ajax {
	...
	public function oembed_handler() {
		...
		if ( empty( $oembed_string ) ) {
			wp_send_json_error( '<p class="ui-state-error-text">' . esc_html__( 'Please Try Again', 'maisonco-core' ) . '</p>' );
		}
	...
	public function get_oembed( $args ) {
		...
		if ( $oembed['embed'] && $oembed['embed'] != $oembed['fallback'] ) {
			return '<div class="cmb2-oembed embed-status">' . $oembed['embed'] . '<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button" rel="' . $oembed['args']['field_id'] . '">' . esc_html__( 'Remove Embed', 'maisonco-core' ) . '</a></p></div>';
		}
		...
		return sprintf(
			...
			sprintf(
				...
				esc_html__( 'No oEmbed Results Found for %1$s. View more info at %2$s.', 'maisonco-core' ),
				$oembed['fallback'],

...
class CMB2_Types {
	...
	public function maybe_custom_field_object( $fieldtype, $args = array() ) {
		if ( $render_class_name = $this->get_render_type_class( $fieldtype ) ) {
			...
			if ( ! ( $this->type instanceof CMB2_Type_Base ) ) {
				throw new Exception( __( 'Custom CMB2 field type classes must extend CMB2_Type_Base.', 'maisonco-core' ) );
			}
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<button type="button" data-selector="<?php echo $table_id; ?>" class="cmb-add-row-button button-secondary"><?php echo esc_html( $this->_text( 'add_row_text', esc_html__( 'Add Row', 'maisonco-core' ) ) ); ?></button>
		</p>
	...
	protected function repeat_row( $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo $class; ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button type="button" class="button-secondary cmb-remove-row-button" title="<?php echo esc_attr( $this->_text( 'remove_row_button_title', esc_html__( 'Remove Row', 'maisonco-core' ) ) ); ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'maisonco-core' ) ) ); ?></button>
			</div>
		</div>

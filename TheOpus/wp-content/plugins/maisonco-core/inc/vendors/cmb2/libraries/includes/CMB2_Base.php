...
abstract class CMB2_Base {
	...
	protected function deprecated_param( $function, $version, $message = null ) {
		...
		if ( is_numeric( $message ) ) {
			...
			switch ( $message ) {
				...
				case self::DEPRECATED_PARAM:
					$message = sprintf( __( 'The "%1$s" field parameter has been deprecated in favor of the "%1$s" parameter.', 'maisonco-core' ), $args[3], $args[4] );
					break;

				case self::DEPRECATED_CB_PARAM:
					$message = sprintf( __( 'Using the "%1$s" field parameter as a callback has been deprecated in favor of the "%1$s" parameter.', 'maisonco-core' ), $args[3], $args[4] );
					break;
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
			if ( function_exists( '__' ) ) {
				if ( ! is_null( $message ) ) {
					trigger_error( sprintf( __( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s! %3$s', 'maisonco-core' ), $function, $version, $message ) );
				} else {
					trigger_error( sprintf( __( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s with no alternative available.', 'maisonco-core' ), $function, $version ) );
				}
			} else {
				if ( ! is_null( $message ) ) {
	...
	public function __get( $field ) {
		switch ( $field ) {
			...
			case 'object_type':
				return $this->{$field};
			default:
				throw new Exception( sprintf( esc_html__( 'Invalid %1$s property: %2$s', 'maisonco-core' ), __CLASS__, $field ) );
		}
	}
	...
	public function __call( $method, $args ) {
		...
		if ( ! has_filter( "{$object_class}_inherit_{$method}" ) ) {
			throw new Exception( sprintf( esc_html__( 'Invalid %1$s method: %2$s', 'maisonco-core' ), get_class( $this ), $method ) );
		}

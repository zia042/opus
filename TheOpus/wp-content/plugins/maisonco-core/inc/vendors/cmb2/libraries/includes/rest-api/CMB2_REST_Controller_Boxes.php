...
class CMB2_REST_Controller_Boxes extends CMB2_REST_Controller {
	...
	public function register_routes() {
		$args = array(
			'_embed' => array(
				'description' => __( 'Includes the registered fields for the box in the response.', 'maisonco-core' ),
			),
		);
		...
		$args['_rendered'] = array(
			'description' => __( 'Includes the fully rendered attributes, \'form_open\', \'form_close\', as well as the enqueued \'js_dependencies\' script handles, and \'css_dependencies\' stylesheet handles.', 'maisonco-core' ),
		);
	...
	public function get_items( $request ) {
		...
		if ( empty( $boxes ) ) {
			return new WP_Error( 'cmb2_rest_no_boxes', __( 'No boxes found.', 'maisonco-core' ), array(
				'status' => 403,
			) );
		}

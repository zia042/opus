...
class CMB2_REST_Controller_Fields extends CMB2_REST_Controller_Boxes {
	...
	public function register_routes() {
		$args = array(
			'_embed' => array(
				'description' => __( 'Includes the box object which the fields are registered to in the response.', 'maisonco-core' ),
			),
			'_rendered' => array(
				'description' => __( 'When the \'_rendered\' argument is passed, the renderable field attributes will be returned fully rendered. By default, the names of the callback handers for the renderable attributes will be returned.', 'maisonco-core' ),
			),
			'object_id' => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'maisonco-core' ),
			),
			'object_type' => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'maisonco-core' ),
			),
	...
	public function update_item( $request ) {
		...
		if ( ! $this->request['value'] ) {
			return new WP_Error( 'cmb2_rest_update_field_error', __( 'CMB2 Field value cannot be updated without the value parameter specified.', 'maisonco-core' ), array(
				'status' => 400,
			) );
		}
	...
	public function modify_field_value( $activity ) {
		...
		if ( ! $this->request['object_id'] || ! $this->request['object_type'] ) {
			return new WP_Error( 'cmb2_rest_modify_field_value_error', __( 'CMB2 Field value cannot be modified without the object_id and object_type parameters specified.', 'maisonco-core' ), array(
				'status' => 400,
			) );
		}
		...
		if ( ! $this->field ) {
			return new WP_Error( 'cmb2_rest_no_field_by_id_error', __( 'No field found by that id.', 'maisonco-core' ), array(
				'status' => 403,
			) );
		}
	...
	protected function prepare_field_data( CMB2_Field $field ) {
		...
		foreach ( $field_args as $key => $value ) {
			...
			if ( empty( $value ) || is_scalar( $value ) || is_array( $value ) ) {
				...
				$field_data[ $key ] = sprintf( __( 'Value Error for %s', 'maisonco-core' ), $key );
			}
		}

...
class CMB2 extends CMB2_Base {
	...
	public function __construct( $config, $object_id = 0 ) {
		...
		if ( empty( $config['id'] ) ) {
			wp_die( esc_html__( 'Metabox configuration is required to have an ID parameter.', 'maisonco-core' ) );
		}
	...
	public function render_group_row( $field_group ) {
		...
		if ( $field_group->args( 'repeatable' ) ) {
			...
			echo '
			<div class="cmbhandle" title="' , esc_attr__( 'Click to toggle', 'maisonco-core' ), '"><br></div>
